...
stringASM = emitBytes .  BytesLiteral . Data.ByteString.Lazy.Char8.pack

-- Emit specific literal bytes in ASM
bytesASM :: (Bounded t_addr, Integral t_addr) => [Word8] -> ASM t_addr ()
bytesASM = emitBytes . BytesLiteral . BS.pack

emitBytes :: (Bounded t_addr, Integral t_addr) => ASMBytes t_addr -> ASM t_addr ()
emitBytes bytes = do
    state <- get
